...
export class RadixTree<T> {
    ...
    insert(word: string, value: T) {
        ...
        while (true) {
            ...
            cosnt nextNode: RadixTreeNode<T> = currentNode.selectNextNodeFromPrefix(sufix);
            if (nextNode === null) {
    ...
    contains (word: string) {
        let currentNode: RadixTreeNod<T> = this.root;
        let cumulatedWord: string = "";
        ...
        return (currentNode !== null && currentNode.is.Leaf() && cumulatedWord.length === word.length);
    }
